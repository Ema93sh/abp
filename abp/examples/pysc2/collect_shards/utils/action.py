...
class ActionWrapper(object):
    ...
    def select(self, actions):
        marines = [unit for unit in self.obs[-1].observation.feature_units if unit.alliance == _PLAYER_SELF]
        ...
        return list(filter(lambda x: x != None, map(lambda action: action_map[action](), actions)))
